...
import { armOf, parseDocKey } from "./documents";
import type { BuiltRetrievers } from "./retrievers";
 ...
 * `weight / (rank + c)` summed per document, with `c = 60`.
 *
 * Rank fusion rather than score fusion, because the arms' scores are not
 * comparable and never will be — BM25 is an unbounded sum over query terms,
 * cosine similarity sits in [-1, 1], trigram similarity in [0, 1]. Normalising
 * them against each other would mean inventing an exchange rate that changes
 * with every query. What each arm asserts is an *order*, and that is all RRF
 * uses. The result is that agreement wins: a row every arm puts near the top
 * beats one that a single arm loves and the others have never heard of.
 *
 * Three things the ensemble does not do, which have to be put back:
 *
...
/** The ensemble's rank-smoothing constant. Matches its default. */
const RRF_C = 60;
...
export type FusedHit = {
  ...
  itemId: string;
  score: number;
  /** Which arms found this row. Reaches the client, and drives goal promotion. */
  arms: SearchArm[];
};
...
class ArmRankingCollector extends BaseCallbackHandler {
  ...
  readonly rankings = new Map<SearchArm, string[]>();
  ...
  handleRetrieverEnd(
    ...
    const arm = armOf(documents[0] ?? { pageContent: "", metadata: {} });
 ...
 */
export async function fuseArms(built: BuiltRetrievers, query: string): Promise<FusedHit[]> {
  ...
  const armsByKey = new Map<string, SearchArm[]>();
